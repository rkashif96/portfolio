...
const Project = (props) => {
    ...
    return (
        <div className='bg-transparent flex flex-col lg:flex-row mt-16'>
            <div className='flex-none w-full lg:w-1/3'>
                ...
                >
                    <img className='w-full lg:w-60 mt-6 lg:mt-14 h-36 lg:h-36 mx-auto lg:ml-6 block rounded-lg' src={props?.img} alt={props?.title} />
                </motion.div>
            </div>
            <motion.div
                ref={textRef}
                ...
                className="text-wrap text-left bg-clip-text text-transparent bg-gradient-to-r from-indigo-300 via-sky-300 to-emerald-300 hover:text-white mt-4 lg:mt-12"
            >
                <p className="font-sans font-bold text-lg sm:text-xl lg:text-2xl bg-clip-text text-transparent bg-gradient-to-r from-indigo-300 via-sky-300 to-emerald-300 hover:text-white">{props?.title}</p>
                ...
                <div className="text-xs sm:text-sm lg:text-base text-white mt-4 flex flex-wrap">
                    {props?.skills.map((skill, index) => (
                        <div key={index} className='rounded-full border border-transparent hover:border-red-500 ml-2 p-2'>
                            <p className='bg-clip-text text-transparent bg-gradient-to-r from-indigo-300 via-sky-300 to-emerald-300 hover:from-purple-400 hover:via-pink-500 hover:to-red-500'>{skill}</p>
                        </div>
                    ))}
                </div>
            </motion.div>
